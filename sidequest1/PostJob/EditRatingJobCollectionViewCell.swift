//
//  EditRatingJobCollectionViewCell.swift
//  sidequest1
//
//  Created by Ken Chiem on 2/3/23.
//

import UIKit

class EditRatingJobCollectionViewCell: UICollectionViewCell {
    // Set Up Variables
    var gigName = UILabel()
    var gigAmount = UILabel()
    var datePosted = UILabel()
    var payButton = UIButton()
    var editRatingButton = UIButton()
...
//        payButton.addTarget(self, action: #selector(presentEditScreen), for: .touchUpInside)
        ...
        editRatingButton.setTitle("Edit QuestTaker Rating", for: .normal)
        editRatingButton.titleLabel?.font = UIFont(name: "Merriweather-Regular", size: 24)
        editRatingButton.layer.backgroundColor = UIColor(red: 0.431, green: 0.729, blue: 0.729, alpha: 1).cgColor
        editRatingButton.addTarget(self, action: #selector(presentRateVC), for: .touchUpInside)
        editRatingButton.layer.cornerRadius = 16
        contentView.addSubview(editRatingButton)
        ...
        editRatingButton.snp.makeConstraints { (make) -> Void in
            make.width.equalTo(336)
            make.height.equalTo(60)
        ...
        archiveButton.snp.makeConstraints { (make) -> Void in
            make.width.equalTo(336)
            make.height.equalTo(60)
            make.top.equalTo(editRatingButton.snp.bottom).offset(12)
            make.centerX.equalTo(contentView.snp.centerX)
        }
